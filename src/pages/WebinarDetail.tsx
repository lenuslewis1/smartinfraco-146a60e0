...
import { findWebinarBySlug, webinars } from "@/data/webinars";
import NotFound from "./NotFound";

export default function WebinarDetailPage() {
  const { slug } = useParams();
  const webinar = findWebinarBySlug(slug);

  if (!webinar) {
    return <NotFound />;
  }

  const related = webinars.filter((item) => item.slug !== webinar.slug).slice(0, 3);
        ...
        <section className="relative min-h-[92svh] overflow-hidden bg-secondary text-white">
          ...
          <div className="relative mx-auto flex min-h-[92svh] w-full max-w-[1500px] flex-col justify-end px-5 pb-14 pt-36 sm:px-8 lg:px-12 lg:pb-20">
            <Reveal className="max-w-5xl">
              <Link to="/webinars" className="inline-flex items-center gap-2 text-sm font-semibold text-white/72 transition hover:text-primary">
                <ArrowLeft className="h-4 w-4" />
                Back to webinars
              </Link>
              <div className="mt-10 flex flex-wrap items-center gap-4 text-xs font-semibold uppercase tracking-[0.18em] text-white/58">
                <Eyebrow tone="accent">{webinar.category}</Eyebrow>
              ...
              <div className="mt-10 flex flex-wrap items-center gap-4 text-sm text-white/68">
          ...
          <div className="relative mx-auto grid w-full max-w-[1500px] gap-14 px-5 sm:px-8 lg:grid-cols-[minmax(0,0.72fr)_minmax(320px,0.28fr)] lg:px-12">
            ...
            <aside className="lg:sticky lg:top-28 lg:self-start">
              <div className="border border-hairline bg-white p-7 shadow-[0_18px_70px_hsl(225_50%_12%_/_0.08)]">
                <Eyebrow tone="primary">Session takeaways</Eyebrow>
                <ul className="mt-7 space-y-5">
                  {webinar.takeaways.map((takeaway) => (
      ...
      <section className="bg-white py-20 lg:py-28">
        <div className="mx-auto w-full max-w-[1500px] px-5 sm:px-8 lg:px-12">
          <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
            <div>
              <Eyebrow tone="muted">More sessions</Eyebrow>
              <h2 className="mt-5 font-display text-4xl font-semibold leading-tight text-foreground lg:text-6xl">
                Related webinars
              </h2>
            </div>
            <button className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground transition hover:text-primary">
            ...
            {related.map((item) => (
              ...
              >
                ...
                <div className="p-7">
                  ...
                  <h3 className="mt-5 font-display text-2xl font-semibold leading-tight text-foreground">{item.title}</h3>
